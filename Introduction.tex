...
Despite their prominence, these particles are never observed in isolation under standard terrestrial conditions.
...
After a few fm/$c$, the quarks and gluons cool down and hadronize into pions, kaons, and many other particles. These hadrons scatter off each other for a few more fm/$c$, until the become so dilute that they free-stream to a detector.}
...
In the subsequent hadronic phase, the hadrons scatter off each other in an elaborate billiards game, with particles near the edge of the expanding fireball having the possibility to free-stream to the detector.
...
The field tries to answer questions such as "When does hadronization occur?", "What equations of state describe the evolution of the interaction region?", "What interactions are occuring between particles during this evolution?", and "What is the size and shape of the medium at kinetic freezeout?".
...
ALICE is one of several experiments at the Large Hadron Collider (LHC).
...
Protons and ions are injected into the LHC ring in bunches of up to $10^{11}$ particles, with bunches traveling in both clockwise and counterclockwise directions around the ring \cite{Lefevre:1092437}.
...
Those bunches are steered around the ring with magnets as strong as 8 Tesla.
...
The main barrel of the detector is 14.1 m long and 15.8 m in diameter. The outer layer (shown in red in the cartoon), is a 0.5 T solenoidal magnet.
...
The innermost detector is the Inner Tracking System (ITS), which provides full azimuthal coverage with a radial range of 3.9 cm to 43 cm.
The ITS consists of six layers of silicon detectors: two Silicon Pixel Detectors, two Silicon Drift Detectors, and two Silicon Strip Detectors.
...
Charged particles traveling through the detector leave behind tracks that the detector can measure, each of which is depicted by a colored line.
...
The TOF can successfully distinguish between particles with momentum as low as 0.5 GeV/$c$ up to 3-4 GeV/$c$, depending on the particle, and it is essential for PID of particles above the 1 GeV/$c$ limit of the TPC. 
...
The number of participants can be estimated using detectors such as the ZDC, which measure the number of spectator nucleons (nucleons that are not participants) in a collision.
...
\end{equation}
and $p$ and $p_\mathrm{z}$ are a particle's three-vector momentum and momentum along the beam axis, respectively.
...
\caption[Monte Carlo Glauber impact parameter simulation]{Distribution of $\sqrt{s_\mathrm{NN}} = 2.76$ TeV Pb-Pb collision impact parameters $b$ from a Glauber Monte Carlo simulation \cite{Abelev:2013xaa}.
...
\caption[Monte Carlo Glauber number of participants simulation]{A Glauber Monte Carlo simulation of the total number of nucleons involved in $\sqrt{s_\mathrm{NN}} = 2.76$ TeV Pb-Pb collisions \cite{Abelev:2013xaa}.
...
At LHC energies, the colliding ions are Lorentz length-contracted by a factor of $\sim 1500$, effectively making them flat disks or pancakes with non-uniform density (more nucleons in the center than toward the edge).
...
Using a Fourier decomposition of the azimuthally-dependent momentum distribution \cite{Snellings:2011sz}, anisotropic flow can be quantified in terms of flow coefficients $v_n$
\begin{equation}
...
This has led some to declare that the quark-gluon plasma is the "most perfect fluid" \cite{Heinz:2005zg}, due to its adherence to ideal fluid dynamics.
...
There are a few ways to dissect spectra.
...
That said, the proton and antiproton yields both lie a couple of sigma below the thermal value, and studies are ongoing to determine what physics effects might drive these particles out of equilibrium.
