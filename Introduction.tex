...
At energies seen by the Large Hadron Collider (LHC), where the ions have center of mass energies per nucleon on the order of TeV, these collisions can produce a hot, dense medium from which thousands of particles are emitted \cite{Aamodt:2010cz}.
This medium is colloquially called a fireball, with more general names being "interaction region" or "particle-emitting source".
The collision, the medium, and the subsequently emitted and measured particles are collectively known as an event. 

For a short time after the initial collision ($\sim$ fm/$c$ = $10^{-15} \mathrm{m}/c \sim 10^{-23}$ seconds), there exists a hot, dense medium called the quark-gluon plasma \cite{Heinz:2000bk}.
...
% In this thesis...
In this thesis, we discuss how femtoscopy is capable of shedding light on both the spatial characteristics of the particle-emitting source, as well as the interactions between particles created within the source.
...
% Chiral perturbation theory offers some scattering parameter predictions \cite{Mai:2009ce}.
