...
% description of therminator, thermal yields, hypersurfaces, parameters, rapidity
...
%The results of the calculations can be found in Section \ref{sec:LambdaParamEstimates}.
...
%Instead, the yields are tempered by reconstruction efficiencies, which naturally results in smaller measured yields and thus smaller numbers of pairs.
%In the present analysis, the $\Lambda$ topological reconstruction cuts have been selected to preferentially remove secondary $\Lambda$ coming from $\Xi$ and $\Omega$ decays (see Section \ref{sec:Recon}).
%Thus, the reconstruction efficiency of a $\Lambda$ depends on the $\Lambda$'s origin.
%To account for the effects of reconstruction efficiency on the $\lambda$ parameters, we artificially introduce reconstruction efficiencies into the THERMINATOR data. 
%Section \ref{sec:ReconstructionEff} describes how the efficiencies have been estimated. In short, we find that $\Lambda$ of all backgrounds have an approximately 15\% chance of being reconstructed.
