\documentclass[ALICE,manyauthors]{ALICE_internal_notes}
%\documentclass[ALICE,manyauthors]{ALICE_scientific_notes}
%
%\newcommand{\jpsi}{\rm J/$\psi$}
%\newcommand{\psip}{$\psi^\prime$}
%\newcommand{\jpsiDY}{\rm J/$\psi$\,/\,DY}
%\newcommand{\dd}{\mathrm{d}}
%\newcommand{\chic}{$\chi_{\rm c}$}
%\newcommand{\ezdc}{$E_{\rm ZDC}$}
%\newcommand{\red}{\textcolor{red}}
%\newcommand{\blue}{\textcolor{blue}}
%\newcommand{\slfrac}[2]{\left.#1\right/#2}
\usepackage{rotating}
\usepackage{comment} 
%\usepackage{epsfig}
%\usepackage{epstopdf}
%\usepackage{graphicx}
%
\begin{document}%
%%%%%%%%%%%%% ptdr definitions %%%%%%%%%%%%%%%%%%%%%
%
%%%%%%%%%%%%%%%  Title page %%%%%%%%%%%%%%%%%%%%%%%%
%
\begin{titlepage}
%
%\EXPnumber{ALICE-INT-2012-xxx} %\EXPdate{31 October 2011}
%\PHdate{\today}
%
%
%%% Put your own title + short title here:
\title{Two-Lambda femtoscopy correlations in PbPb collisions at $\sqrt{s_{NN}}=2.76$ TeV from the ALICE experiment}
\ShortTitle{Two-Lambda femtoscopy correlations in PbPb collisions at $\sqrt{s_{NN}}=2.76$ TeV}   % appears on right page headers
%
\author{Jai Salzwedel$^{1}$}
\author{
1. The Ohio State University, Columbus, USA\\
}
\author{Email: jai.salzwedel@cern.ch}
%
\ShortAuthor{ALICE Internal Note 2013}      % appears on left page headers, do not change
%
\begin{abstract}
Two-$\Lambda$ momentum correlation functions measured in PbPb collisions at $\sqrt{s_{NN}}=2.76$ TeV by the ALICE collaboration are presented.  Results are shown for $\Lambda\Lambda$, $\bar{\Lambda}\bar{\Lambda}$ and $\Lambda\bar{\Lambda}$ pairs.
\end{abstract}
\end{titlepage}
%

\input{AnalysisNote.tex}               %%%%%%%%%%% put the body of the article here

\end{document}
